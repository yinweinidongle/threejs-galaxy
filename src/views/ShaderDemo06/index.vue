...
import flowVertexShader from './shader/shipflow/vertex.glsl?raw'
import flowFragmentShader from './shader/shipflow/fragment.glsl?raw'
import gpgpuParticlesShader from './shader/gpgpu/particles.glsl?raw'
...
let particles,particles0,baseGeometry;

let gpgpu,sphereMaterial;
...
gpgpu = {}
 gpgpu.size = Math.ceil(Math.sqrt(baseGeometry.count)) 
     ...
     gpgpu.computation = new GPUComputationRenderer(gpgpu.size,gpgpu.size,base.renderer)
     ...
     base.scene.add(gpgpu.debug)

   
    console.log(particles0.geometry)
...
particles0.geometry = new THREE.BufferGeometry()
particles0.geometry.setDrawRange(0,baseGeometry.count)
particles0.geometry.setAttribute('aParticlesUv',new THREE.BufferAttribute(particlesUvArray,2))
    ...
    sphereMaterial = new THREE.ShaderMaterial( { 
        depthWrite:false,
            ...
            uniforms:{
                    uniforms:{
                    uTime:{value:0},
                    uSize:new THREE.Uniform(15.0),
                    uParticlesTexture:new THREE.Uniform(gpgpu.computation.getCurrentRenderTarget(gpgpu.particlesVariable).texture),
     ...
     console.log(particles0.geometry)
     particles0.mesh = new THREE.Points( sphereGeometry, sphereMaterial ); base.scene.add( particles0.mesh );
...
function update() {
    ...
    sphereMaterial.uniforms.uParticlesTexture = gpgpu.computation.getCurrentRenderTarget(gpgpu.particlesVariable).texture
